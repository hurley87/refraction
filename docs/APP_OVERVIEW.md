...
- refreshed landing page with **desktop layout** (dedicated home nav, hero city carousel, artist CTA, redesigned city guides cover + carousel, IRL tour with admin-pinnable featured event) -> interactive map (`Find Spots Nearby`) and **city guides** entry points
...
5. **App action:** Create or update the player record, initialize `total_points`, fire `account_created` only for net-new players (with optional **signup attribution** from the client when present), and run non-blocking signup syncs for downstream systems.
6. **Success means:** The user has a persisted `players` row with wallet, username, and a usable points balance.

### Discovering a location, checkpoint, venue, or guide

1. **User action:** Open the interactive map, the events page, the city guides hub, or a shared checkpoint URL at `/c/[id]`.
2. **App action:** Load visible locations for the map, merge DICE events with admin-managed manual events from Supabase for the events page (with optional city and date filters), load published guides/editorials from Supabase for the city guides hub (with optional city filter), or load the active checkpoint configuration.
3. **User action:** Pick a place to visit, a guide to read, or a checkpoint to use.
4. **Success means:** The user reaches a valid surface where they can check in, redeem a spend item, or browse a venue, event, or guide.

### Completing a location check-in

1. **User action:** Select or submit a location and attempt a location check-in.
2. **App action:** Create or fetch the player and location, reject duplicate check-ins, and reject non-creators from checking in to legacy hidden locations.
...
2. **App action:** Load active `perks`, current player balance, `tiers`, redemption state, and available code information; users can filter the catalog by city and category.
...
4. **App action:** Track `reward_claim_clicked` on redeem intent, then `reward_claimed` after success.
...
| Interactive map                          | Shows visible locations and serves as the main discovery surface for check-ins; includes saved-location favorites, responsive drawer UX tuned for desktop/4K, and an in-progress desktop landing handoff.                                                                                                                                                                                                                                                                                                                              | End users                        | `live` |
| Map location favorites                   | Lets signed-in users heart/save locations from map cards and the check-in flow; persists rows in `player_location_favorites`, surfaces a Favorites list in the map drawer, and shows saved spots on the dashboard. Requires a connected wallet.                                                                                                                                                                                                                                                                                        | End users                        | `live` |
...
| Rewards catalog                          | Lists active rewards across `/rewards` and `/perks` with eligibility and expiration context; users can filter by city and category; admins can pin a **featured** perk (`is_featured`) to the hero slot and the list sorts ending-soon perks toward the top. Logged-out users see a **Join IRL** CTA on reward detail.                                                                                                                                                                                                                 | End users                        | `live` |
| Perk claiming                            | Lets an eligible user redeem a perk and receive a code or claim link.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | End users                        | `live` |
| Spend redemption                         | Supports point-spend items tied to checkpoints, pending verification, instant redemption, and completion tracking. Client and server resolve a valid EVM wallet when Privy's primary linked account is Stellar/Solana/Aptos so alt-chain check-in users can still spend. The Base and Stellar USDC pilot adds server-side conversion and payment orchestration, scheduled pending-operation reconciliation (Vercel Cron), opportunistic reconcile on some reads, rail-aware receipts, and richer Mixpanel fields for rail diagnostics. | End users                        | `live` |
| Events page                              | Shows public events merged from DICE plus manual events stored in Supabase and edited through admin routes; users can filter by city and date range. Admins can feature one manual or DICE event on the homepage IRL Tour section.                                                                                                                                                                                                                                                                                                     | End users + internal team        | `live` |
| Cities admin + event city tagging        | Admins manage a `cities` table and assign manual events to cities; public `/api/cities` powers event-page and rewards city filters.                                                                                                                                                                                                                                                                                                                                                                                                    | Internal team                    | `live` |
| Categories admin                         | Admins manage a canonical `categories` table (16 seeded types) via `/admin/categories`; public `/api/categories` powers perks typing and rewards category filters.                                                                                                                                                                                                                                                                                                                                                                     | Internal team                    | `live` |
| Signup CRM sync                          | Sends users with email to Campaign Monitor Onboarding on net-new signup (`/api/player`) or on first successful check-in (location or checkpoint), in a non-blocking step.                                                                                                                                                                                                                                                                                                                                                              | Internal team                    | `live` |
| City guides + editorials                 | Serves published city guides and editorial pages from a Supabase-backed CMS, including guide-to-map handoff and hub city filtering.                                                                                                                                                                                                                                                                                                                                                                                                    | End users                        | `live` |
| Guides CMS + draft preview               | Lets admins create, edit, publish, feature, and preview guides/editorials before they go live.                                                                                                                                                                                                                                                                                                                                                                                                                                         | Internal team                    | `live` |
| User-submitted locations                 | Lets users create locations that now go live on the map immediately when the place is unique.                                                                                                                                                                                                                                                                                                                                                                                                                                          | End users + admins               | `live` |
| Public Records sponsored USDC activation | Sponsor-funded venue activations: attendees confirm purchase and swipe to redeem perks at `/activation/[id]` without user-custody USDC. Campaign wallet settles to venue on **Base** or **Stellar** per activation; async settlement via Vercel Cron (`/api/cron/sponsored-settlement`), admin CRUD + ops dashboard (including full campaign-wallet USDC withdraw/refund), refreshed multi-stage attendee UX, and Mixpanel funnel (IRL-51–62).                                                                                         | End users + sponsors + operators | `live` |
| Admin operations surfaces                | Admin routes exist for users, locations (including CSV export), checkpoints, perks, categories, manual events (including homepage featured-event pinning), analytics, city metrics, sponsored activations, and related operations.                                                                                                                                                                                                                                                                                                     | Internal team                    | `live` |
...
| Events ingestion               | Event discovery is live, but the feed is hybrid: DICE API plus manual events maintained in Supabase through admin tools (not a single fully automated source).                                                                                                                                                  | End users + internal team   | `manual`  |
...
| Sponsored activation pilot ops | Attendee and settlement flows are implemented, but launch depends on per-activation config (rails, campaign/venue wallets, caps) and operator runbooks. Admin campaign-wallet withdraw refunds the full on-chain balance (including reserved USDC); on Stellar shared wallets this can affect peer activations. | Internal team + venue staff | `manual`  |
...
- Stores persisted product state: users, points balances, locations, check-ins, checkpoints, perks, codes, redemptions, tiers, guides CMS content, and admin-facing analytics queries.
...
- Stores behavioral analytics: pageviews, activation events, reward views/claims, spend redemptions, **sponsored activation funnel** events (view → eligibility → confirm → swipe → settlement), location creation, tier progression, and **signup / acquisition attribution** (first- and last-touch UTMs, referrer, landing URLs, checkpoint context).
- Trust Mixpanel for engagement trends, funnels, behavior sequencing, and **marketing attribution** when events are instrumented.
- **Historical caveat:** `account_created` attribution fields are only reliable for signups that occur after first-touch capture shipped; older signups may lack normalized `signup_source` / channel data.
- Limitations:
...
- Stores marketing-list subscriptions for users with email when they complete signup or their first check-in.
- Trust Campaign Monitor for email audience sync state, not product analytics or source-of-truth user/account facts.
- Onboarding list sync runs on `account_created` (signup) and on first `checkin_completed` when an email is known — event check-ins often skip signup, so check-in is required for tour/event audiences.
- Limitations:
  - Sync requires an email at signup or first check-in; check-ins without email do not subscribe.
  - The sync is non-blocking, so signup/check-in can still succeed if Campaign Monitor fails.
...
| `player_location_favorites`     | table          | Supabase          | Per-player saved map locations.                                                               | Written when a user favorites a location via `/api/location-favorite`.                   | Requires a resolved `locations` row for the `place_id`; no Mixpanel event is emitted today.                                                                                                                                                                                                                                                                                                                                                                |
| `locations`                     | table          | Supabase          | Discoverable locations or venues.                                                             | Created through location creation or location check-in flows.                            | Public surfaces typically use visible locations only; new user-created rows are now inserted visible, but some legacy rows can still be hidden.                                                                                                                                                                                                                                                                                                            |
| `guides`                        | table          | Supabase          | CMS records for published/draft city guides and editorials.                                   | Created and updated through admin guides routes.                                         | `kind` distinguishes `city_guide` vs `editorial`; draft previews rely on signed preview tokens.                                                                                                                                                                                                                                                                                                                                                            |
| `guide_contributors`            | table          | Supabase          | Ordered contributor metadata attached to a guide or editorial.                                | Written through admin guides editing flows.                                              | Display-only content; ordering is positional per guide.                                                                                                                                                                                                                                                                                                                                                                                                    |
| `guide_location_overrides`      | table          | Supabase          | Optional per-guide contributor labels for linked venue cards.                                 | Written when admins customize guide-linked locations.                                    | Applies only to city guides; falls back to guide contributors when unset.                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `account_created`               | event          | Mixpanel          | A new player profile was created.                                                             | Fired only when `/api/player` creates a net-new player.                                  | Includes normalized first-touch fields (`signup_source`, `signup_channel`, optional `signup_context`) plus raw initial/current UTMs, referrer, landing URLs, path, and optional `checkpoint_id` / `event_id` / `location_id` when captured. When the client sends no usable attribution, `signup_source` / `signup_channel` default to `direct`. `wallet_type` is still sent as `EVM` from this route. Historical signups may predate attribution capture. |
...
| `points_earned`                 | event          | Mixpanel          | Points were awarded for an action.                                                            | Fired for checkpoint check-ins, location check-ins, and location creation.               | Does not represent point spending. Points amount is on the `amount` property. `activity_type` values are `checkpoint_checkin`, `location_creation`, and `daily_checkin` — note that `daily_checkin` is the label emitted for GPS **location check-ins** (not a separate daily-streak feature), so do not interpret it as a distinct product surface.                                                                                                       |
...
| `reward_claim_clicked`          | event          | Mixpanel          | A user tapped redeem/claim on a reward detail.                                                | Fired client-side on redeem intent across perk detail and rewards list flows.            | Use with `reward_page_viewed` and `reward_claimed` to measure claim intent vs completion.                                                                                                                                                                                                                                                                                                                                                                  |
...
| `spend_redemption_started`      | event          | Mixpanel          | A spend redemption was initiated.                                                             | Fired on the pending-create spend flow.                                                  | Optional `spend_source` / `transaction_context` (checkpoint flows use `event` / `checkpoint`). Not all instant spend paths emit a start event first.                                                                                                                                                                                                                                                                                                       |
| `spend_redemption_completed`    | event          | Mixpanel          | A spend redemption was fulfilled or verified.                                                 | Fired on verify, admin fulfill, or instant redemption completion.                        | Same optional spend segmentation fields as `spend_redemption_started`. Better completion signal than `spend_redemption_started`.                                                                                                                                                                                                                                                                                                                           |
...
| `location_created`              | event          | Mixpanel          | A location was created.                                                                       | Fired after successful location creation and some admin CSV imports.                     | New user-created locations are now created visible by default, but legacy/admin rows can still differ.                                                                                                                                                                                                                                                                                                                                                     |
| `sponsored_activation_viewed`   | event          | Mixpanel          | Public activation page opened.                                                                | Fired when the attendee activation surface loads.                                        | Part of the Public Records sponsored activation funnel (IRL-61); distinct from user-wallet spend pilot events.                                                                                                                                                                                                                                                                                                                                             |
| `sponsored_redemption_redeemed` | event          | Mixpanel          | Attendee completed swipe-to-redeem at venue.                                                  | Fired after successful swipe redemption.                                                 | Settlement may still be async; pair with `sponsored_settlement_*` events for onchain proof.                                                                                                                                                                                                                                                                                                                                                                |
...
- **Supabase** remains the source of truth for persisted product facts and completed actions:
  - current points balances
  - completed location check-ins
  - completed perk claims
  - spend redemption state
  - active locations, checkpoints, tiers, rewards, guides, and editorials
- **Mixpanel** is for behavioral analytics, funnel analysis, and **attribution** (how users arrived and moved through flows)—not for balances or authoritative counts of completed writes.
...
| Reward interest -> reward claim          | `reward_page_viewed`       | `reward_claimed` (via `reward_claim_clicked`)                        | Measures whether reward demand converts into claimed value; `reward_claim_clicked` isolates redeem intent. |
...
- New user-created locations are now created visible by default, but some legacy hidden locations still exist and remain creator-only for check-ins until visibility changes.
- Location creation is capped per creator per week.
- Events data is hybrid: DICE API plus manual events stored in Supabase and edited through admin routes; homepage featured events resolve live from DICE GraphQL or a pinned manual row.
- Perk categories are admin-managed (`categories` table) but not yet wired to map/location filters.
- Challenges / quests are not fully productized; current progress/completion is placeholder behavior.
- City guides and editorials are now CMS-backed, but publishing quality and curation are still admin-operated.
- Spend redemption can involve manual or venue-side verification/fulfillment steps.
- Map location favorites have no dedicated Mixpanel events; engagement must be inferred from Supabase row counts or product QA.
...
- Do not recommend suggesting city guides as a net-new idea; a CMS-backed guides/editorials system already exists.
- Do not recommend suggesting challenges as a net-new idea; challenge surfaces already exist, but the current implementation is still partial/placeholder.
- Do not recommend building sponsor-funded venue activations with campaign-to-venue USDC settlement from scratch; the Public Records pilot stack (APIs, `/activation/[id]` UI, settlement workers, admin dashboard) already exists—focus on config, ops, and measured rollout instead.
