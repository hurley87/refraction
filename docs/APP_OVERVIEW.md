...
| `points_earned`              | event          | Mixpanel          | Points were awarded for an action.                                                            | Fired for checkpoint check-ins, location check-ins, and location creation.               | Does not represent point spending. The `activity_type` values are `checkpoint_checkin`, `location_creation`, and `daily_checkin` — the `daily_checkin` label is emitted for location check-ins (not a separate daily streak event). The numeric points are on the `amount` property; `points` is currently always 0. |
