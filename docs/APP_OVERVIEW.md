...
| `points_activities`          | table          | Supabase          | Stored point-earning activity rows.                                                           | Written for checkpoint check-ins, location creation, and one-time profile-field bonuses (`profile_field_*` activity types from `/api/profile`). | Not a complete ledger for all point changes; GPS location check-ins update `player_location_checkins` and `players.total_points` but do not always appear here. Profile-field rows are Supabase-only (no `points_earned` Mixpanel event).                                                                                                                                                                                                                  |
