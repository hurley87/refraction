...
2. **App action:** Load visible locations for the map, merge DICE events with admin-managed manual events from Supabase for the events page, load published guides/editorials from Supabase for the city guides hub, or load the active checkpoint configuration.
...
| Spend redemption                         | Supports point-spend items tied to checkpoints, pending verification, instant redemption, and completion tracking. The Base and Stellar USDC pilot adds server-side conversion and payment orchestration, scheduled pending-operation reconciliation (Vercel Cron), opportunistic reconcile on some reads, rail-aware receipts, and richer Mixpanel fields for rail diagnostics. | End users                        | `live` |
| Events page                              | Shows public events merged from DICE plus manual events stored in Supabase and edited through admin routes.                                                                                                                                                                                                                                                                      | End users                        | `live` |
...
| Sponsored activation pilot ops | Attendee and settlement flows are implemented, but launch depends on per-activation config (rails, campaign/venue wallets, caps) and operator runbooks.        | Internal team + venue staff | `manual`  |
