...
| 7.2 | [ ] When `spend_transactions.explorer_tx_url` is persisted, link matches that **persisted** URL (preferred).                                                                                                   | [ ] Same precedence.                                                                                                                                                                 |
| 7.3 | [ ] When persisted URL absent, link matches `NEXT_PUBLIC_SPEND_RAIL_BASE_USDC_EXPLORER_TX_URL_TEMPLATE` with `{txHash}` substituted for the recorded payment hash (lowercased `0x…` per client helper).        | [ ] Fallback uses `NEXT_PUBLIC_SPEND_RAIL_STELLAR_USDC_EXPLORER_TX_URL_TEMPLATE` (or server default derived from public network metadata—verify against configured template in env). |
