...
describe('API Schemas', () => {
  ...
  describe('leaderboardQuerySchema', () => {
    ...
    it('should reject non-positive playerId', () => {
      expect(leaderboardQuerySchema.safeParse({ playerId: 0 }).success).toBe(
        false
      );
      expect(leaderboardQuerySchema.safeParse({ playerId: -1 }).success).toBe(
        false
      );
    });
  });
  ...
  describe('locationCheckinRequestSchema', () => {
    ...
    it('should reject non-positive locationId', () => {
      expect(
        locationCheckinRequestSchema.safeParse({
          walletAddress: '0x1234567890abcdef1234567890abcdef12345678',
          locationId: 0,
        }).success
      ).toBe(false);
  ...
  describe('locationCommentsQuerySchema', () => {
    ...
    it('should coerce locationId from string', () => {
      const result = locationCommentsQuerySchema.safeParse({
        locationId: '456',
      });
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it('should reject non-positive locationId', () => {
      expect(
        locationCommentsQuerySchema.safeParse({ locationId: 0 }).success
      ).toBe(false);
      expect(
        locationCommentsQuerySchema.safeParse({ locationId: -1 }).success
      ).toBe(false);
    });
  });
  ...
  describe('chainTypeSchema', () => {
    it('should accept valid chain types', () => {
      ...
      expect(chainTypeSchema.safeParse('aptos').success).toBe(true);
    });
  ...
  describe('updateCheckpointRequestSchema', () => {
    ...
    it('should reject invalid values when provided', () => {
      expect(
        updateCheckpointRequestSchema.safeParse({ name: '' }).success
      ).toBe(false);
      expect(
        updateCheckpointRequestSchema.safeParse({ points_value: 0 }).success
      ).toBe(false);
      expect(
        updateCheckpointRequestSchema.safeParse({ chain_type: 'invalid' })
          .success
      ).toBe(false);
    });
