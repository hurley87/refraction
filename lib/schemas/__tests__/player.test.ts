...
describe('Player Schemas', () => {
  ...
  describe('solanaWalletAddressSchema', () => {
    ...
    it('should reject address with invalid base58 characters (0, O, I, l)', () => {
      ...
      expect(solanaWalletAddressSchema.safeParse(invalidWith0).success).toBe(false)

      // O is not valid in base58
      const invalidWithO = 'ONd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T'
      expect(solanaWalletAddressSchema.safeParse(invalidWithO).success).toBe(false)

      // I is not valid in base58
      const invalidWithI = 'INd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T'
      expect(solanaWalletAddressSchema.safeParse(invalidWithI).success).toBe(false)

      // l (lowercase L) is not valid in base58
      const invalidWithl = 'lNd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T'
      expect(solanaWalletAddressSchema.safeParse(invalidWithl).success).toBe(false)
    })
    ...
    it('should reject non-string types', () => {
      ...
      expect(solanaWalletAddressSchema.safeParse(undefined).success).toBe(false)
    })
  })
  ...
  describe('stellarWalletAddressSchema', () => {
    ...
    it('should accept another valid Stellar wallet address', () => {
      const validAddress = 'GA7YNBW5CBTJZ3ZZOWX3ZNBKD6OE7A7IHUQVWMY62W2ZBG2SGZVOOPVH'
      const result = stellarWalletAddressSchema.safeParse(validAddress)
      expect(result.success).toBe(true)
    })
    ...
    it('should reject address with wrong length (too long)', () => {
      const invalidAddress = 'GBRPYHIL2CI3FNQ4BXLFMNDLFJUNPU2HY3ZMFSHONUCEOASW7QC7OX2HH'
      const result = stellarWalletAddressSchema.safeParse(invalidAddress)
      expect(result.success).toBe(false)
    })

    it('should reject lowercase characters', () => {
      const invalidAddress = 'gBRPYHIL2CI3FNQ4BXLFMNDLFJUNPU2HY3ZMFSHONUCEOASW7QC7OX2H'
      const result = stellarWalletAddressSchema.safeParse(invalidAddress)
      expect(result.success).toBe(false)
    })
    ...
    it('should reject non-string types', () => {
      ...
      expect(stellarWalletAddressSchema.safeParse(undefined).success).toBe(false)
    })
  })
  ...
  describe('updateUserProfileSchema', () => {
    ...
    it('should reject invalid website URL', () => {
      const result = updateUserProfileSchema.safeParse({ website: 'not-a-url' })
      expect(result.success).toBe(false)
    })

    it('should reject invalid profile picture URL', () => {
      const result = updateUserProfileSchema.safeParse({ profile_picture_url: 'not-a-url' })
      expect(result.success).toBe(false)
    })

    it('should reject handle too long', () => {
      const result = updateUserProfileSchema.safeParse({ twitter_handle: 'a'.repeat(51) })
      expect(result.success).toBe(false)
    })
  })
