...
describe('Location Schemas', () => {
  ...
  describe('createLocationSchema', () => {
    ...
    it('should reject missing required fields', () => {
      expect(createLocationSchema.safeParse({ name: 'Test' }).success).toBe(
        false
      );
      expect(
  ...
  describe('updateLocationSchema', () => {
    ...
    it('should reject invalid values when provided', () => {
      ...
      expect(updateLocationSchema.safeParse({ place_id: '' }).success).toBe(
        false
      );
      expect(updateLocationSchema.safeParse({ latitude: 100 }).success).toBe(
        false
      );
      expect(updateLocationSchema.safeParse({ longitude: 200 }).success).toBe(
        false
      );
    });
    ...
    it('should reject invalid URLs when provided', () => {
      expect(
        updateLocationSchema.safeParse({ coin_image_url: 'not-a-url' }).success
      ).toBe(false);
      expect(
        updateLocationSchema.safeParse({ event_url: 'not-a-url' }).success
      ).toBe(false);
    });
  });
