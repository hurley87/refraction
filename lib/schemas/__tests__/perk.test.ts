...
describe('Perk Schemas', () => {
  ...
  describe('updatePerkSchema', () => {
    ...
    it('should reject invalid values when provided', () => {
      ...
      expect(updatePerkSchema.safeParse({ description: '' }).success).toBe(
        false
      );
      expect(updatePerkSchema.safeParse({ type: '' }).success).toBe(false);
      expect(updatePerkSchema.safeParse({ points_threshold: -1 }).success).toBe(
        false
      );
    });
  ...
  describe('createLocationListSchema', () => {
    ...
    it('should reject invalid hex color format', () => {
      expect(
        createLocationListSchema.safeParse({
          title: 'Title',
          slug: 'valid-slug',
          accent_color: 'red',
        }).success
  ...
  describe('updateLocationListSchema', () => {
    ...
    it('should reject invalid values when provided', () => {
      expect(updateLocationListSchema.safeParse({ title: '' }).success).toBe(
        false
      );
      expect(updateLocationListSchema.safeParse({ slug: '' }).success).toBe(
        false
      );
      expect(
        updateLocationListSchema.safeParse({ slug: 'Invalid' }).success
      ).toBe(false);
      expect(
        updateLocationListSchema.safeParse({ accent_color: 'red' }).success
      ).toBe(false);
    });
  ...
  describe('max_claims_per_member_per_day', () => {
    it('accepts null (unlimited) on create', () => {
      const result = createPerkSchema.safeParse({
        title: 'Welcome Drink',
    ...
    it('accepts a positive daily cap', () => {
      const result = createPerkSchema.safeParse({
        title: 'Welcome Drink',
        description: 'A free drink',
        points_threshold: 100,
    ...
    it('rejects zero or negative caps', () => {
      expect(
        createPerkSchema.safeParse({
          ...
          points_threshold: 100,
          type: 'bar',
          max_claims_per_member_per_day: 0,
        }).success
      ).toBe(false);

      expect(
        updatePerkSchema.safeParse({
          max_claims_per_member_per_day: -1,
        }).success
      ).toBe(false);
    });
