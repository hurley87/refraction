...
const GUIDE_ARTICLE_LINK_BASE_CLASS =
  'font-semibold underline underline-offset-2 hover:opacity-90';

/**
 * Guide / editorial hyperlinks: IRL yellow highlight + dark text (same pattern as article titles).
 * Yellow text on white fails contrast; highlight keeps the brand legible.
 */
export const IRL_YELLOW_HYPERLINK_CLASS = cn(
  GUIDE_ARTICLE_LINK_BASE_CLASS,
  'box-decoration-clone bg-[var(--IRL-Yellow,#FFF200)] px-0.5 py-px text-[#171717]',
  'decoration-[#171717]/45'
);

/** Icon / compact links: yellow chip behind dark glyph. */
export const IRL_YELLOW_TEXT_CLASS =
  'rounded-sm bg-[var(--IRL-Yellow,#FFF200)] p-1 text-[#171717]';

function guideArticleMarkdownComponents(
  hyperlinkClassName?: string
): Components {
  const linkClassName = hyperlinkClassName ?? IRL_YELLOW_HYPERLINK_CLASS;
  ...
  return {
    ...
    p: ({ children }) => (
      <p className="mb-3 first:mt-0 last:mb-0">{children}</p>
    ),
...
export const GUIDE_ARTICLE_MARKDOWN_COMPONENTS =
  guideArticleMarkdownComponents();

export interface GuideArticleMarkdownProps {
  markdown: string;
  className?: string;
  /** When set (e.g. {@link IRL_YELLOW_HYPERLINK_CLASS}), overrides default link colors. */
  hyperlinkClassName?: string;
}
  ...
  return (
    <div className={cn('body-medium text-[#171717]', className)}>
      <ReactMarkdown
        ...
        components={guideArticleMarkdownComponents(hyperlinkClassName)}
      >
        {markdown}
      </ReactMarkdown>
    </div>
