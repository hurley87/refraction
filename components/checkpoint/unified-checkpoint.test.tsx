...
// Mock Aptos wallet hook
const mockUseAptosWallet = vi.fn();
vi.mock('@/hooks/useAptosWallet', () => ({
...
describe('UnifiedCheckpoint', () => {
  ...
  const mockAptosCheckpoint: Checkpoint = {
    ...mockEvmCheckpoint,
    chain_type: 'aptos',
  };
  ...
  describe('No Wallet State', () => {
    describe('EVM Checkpoint', () => {
      it('should show wallet required message for EVM checkpoint without wallet', () => {
        ...
        expect(
          screen.getByText(/please connect your wallet/i)
        ).toBeInTheDocument();
        expect(
    ...
    describe('Solana Checkpoint', () => {
      it('should show create Solana wallet prompt', async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getByRole('heading', { name: /create solana wallet/i })
          ).toBeInTheDocument();
        });
        expect(
          screen.getByRole('button', { name: /create solana wallet/i })
        ).toBeInTheDocument();
      });
    ...
    describe('Stellar Checkpoint', () => {
      it('should show create Stellar wallet prompt', async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getByRole('heading', { name: /create stellar wallet/i })
          ).toBeInTheDocument();
        });
        expect(
          screen.getByRole('button', { name: /create stellar wallet/i })
        ).toBeInTheDocument();
      });
      ...
      it('should show Stellar error message when present', () => {
        ...
        expect(
          screen.getByText('Failed to connect to Stellar')
        ).toBeInTheDocument();
      });
    });

    describe('Aptos Checkpoint', () => {
      it('should show create Aptos wallet prompt', async () => {
        mockUsePrivy.mockReturnValue({
          user: {
            wallet: { address: '0x123' },
            email: { address: 'test@example.com' },
        ...
        await waitFor(() => {
          expect(
            screen.getByRole('heading', { name: /create aptos wallet/i })
          ).toBeInTheDocument();
        });
        expect(
          screen.getByRole('button', { name: /create aptos wallet/i })
        ).toBeInTheDocument();
      });

      it('should call connect when create Aptos wallet button is clicked', async () => {
        const user = userEvent.setup();
        const mockConnect = vi.fn();
        mockUsePrivy.mockReturnValue({
          user: {
      ...
      it('should show Aptos error message when present', () => {
        mockUsePrivy.mockReturnValue({
          user: {
            wallet: { address: '0x123' },
            email: { address: 'test@example.com' },
        ...
        mockUseAptosWallet.mockReturnValue({
          ...
          isConnecting: false,
          isLoading: false,
          error: 'Failed to connect to Aptos',
        });

        render(<UnifiedCheckpoint checkpoint={mockAptosCheckpoint} />);

        expect(
          screen.getByText('Failed to connect to Aptos')
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Auto Check-in', () => {
    ...
    it('should call unified endpoint for Aptos checkpoint', async () => {
      mockUsePrivy.mockReturnValue({
        user: {
          wallet: { address: '0x123' },
          email: { address: 'test@example.com' },
  ...
  describe('Check-in Error', () => {
    ...
    it('should show daily limit message when limit is reached', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/up to 10 checkpoint visits per day/i)
        ).toBeInTheDocument();
      });
    });
    ...
    it('should show Visit IRL.ENERGY button on error', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole('button', { name: /visit irl\.energy/i })
        ).toBeInTheDocument();
      });
    });
  });
