  ...
  try {
    if (!solanaWalletAddress || !checkpoint) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "Solana wallet address and checkpoint are required",
        }),
        {
          status: 400,
    ...
    // Create or link player by Solana wallet (unified by email)
    const player = await createOrUpdatePlayerForSolana(
      solanaWalletAddress,
      email
    );
    ...
    const { count: checkpointCheckinsToday, error: checkpointCountError } =
      await supabase
        ...
        .eq("metadata->>solana_wallet", solanaWalletAddress)
    ...
    const activityData: Record<string, unknown> = {
      ...
      description: `Solana checkpoint visit: ${checkpoint}`,
      metadata: {
        checkpoint,
        email,
        chain: "solana",
        solana_wallet: solanaWalletAddress,
        player_id: player.id,
      },
      processed: true,
    };
    ...
    // Get today's total points by Solana wallet
    ...
    return new Response(
      JSON.stringify({
        ...
        message: `Nice! You earned ${pointsAwarded} points for this Solana checkpoint.`,
      }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
