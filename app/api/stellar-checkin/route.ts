...
import { createOrUpdatePlayerForStellar } from "@/lib/db/players";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!stellarWalletAddress || !checkpoint) {
      return apiError("Stellar wallet address and checkpoint are required", 400);
    }

    const player = await createOrUpdatePlayerForStellar(stellarWalletAddress, email);
    ...
    const result = await processCheckin({
      ...
      email,
      chain: "stellar",
      chainWalletAddress: stellarWalletAddress,
    });
